...
<div>
    ...
    <table className="table-auto w-full">
        ...
        <tr>
            <th colSpan={2} className="w-20 px-5 py-3 tracking-wide">Product</th>
        ...
        {product.map((item, index) => (
            <tr key={index}>
            <td className="h-32 w-32 px-5 py-3 border-b border-neutral">
    ...
    {product.map((item, index) => (
        <div key={index} className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:hidden">
        <div className="space-y-2 p-4 rounded-lg shadow-xl">
            <div><img className="w-32 mx-auto" src={item.image} alt={item.title}/></div>
            <div><p className="font-semibold text-base text-center">{item.title}</p></div>
            <div className="badge badge-default badge-outline text-xs">{item.category}</div>
            <div className="text-sm text-justify">{item.description}</div>
            <div className="flex items-center space-x-2">
                <input type="number" value={item.quantity} className="input input-bordered text-center font-bold text-sm h-8 w-32 mr-2 rounded-base"/>
